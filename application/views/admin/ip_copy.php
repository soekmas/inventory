<?php 
    ...
    foreach ( $ips as $row):
?>
<?php echo form_open(base_url() . 'index.php?admin/ip/add/'.$param2 , array('class' => 'form-horizontal group-border-dashed', 'enctype' => 'multipart/form-data'));?>

<h4 class="text-center">Copy IP information</h4>
<hr>
<div class="form-group">
  <label class="col-sm-3 control-label">Host Name</label>
  <div class="col-sm-6">
    <input type="text"  name="host_name" value="Copy of <?php echo $row['host_name']; ?>" class="form-control" placeholder="Enter host or location or user name" />
  </div>
  <!-- End col-6 --> 
</div>
<!-- end form -group -->
